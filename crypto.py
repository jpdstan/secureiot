"""
Cryptographic functions. Used in another project of mine.
...
class Crypto(object):
    ...
    def gen_asymmetric_keypair(self, size):
        key = RSA.generate(size)
        return key.publickey(), key

    def save_keyfile(self, username, private_key):
    ...
    def load_keyfile(self, username):
        keyfile = os.path.join("keys", username + ".pem")
        private_key = None
    ...
    def remove_keyfile(self, username):
        keyfile = os.path.join("keys", username + ".pem")
        if os.path.exists(keyfile):
...
if __name__ == "__main__":
    ...
    pubkey, key = crypto.gen_asymmetric_keypair(2048)
    crypto.save_keyfile("testuser", key)
    key_loaded = crypto.load_keyfile("testuser")
    ...
    crypto.remove_keyfile("testuser")
